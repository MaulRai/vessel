...
export default function PendanaDashboardPage() {
	const totalPembiayaanBerjalan = useMemo(
		() => portfolio.saldoTersedia + portfolio.danaDisalurkan,
		[]
	);
	...
	const cumulativeStops = donutSegments.reduce<{ stop: number; color: string }[]>(
		(acc, seg) => {
...
function Td({ children, className }: { children: React.ReactNode; className?: string }) {
	return (
		<td
			className={`px-4 py-3 align-top text-sm text-slate-300${className ? ` ${className}` : ""}`}
		>
			{children}
		</td>
	);
