...
function RiskAssessmentContent() {
    ...
    useEffect(() => {
        // Mock-only: load demo questions immediately without checking status or API
    ...
    const handleSubmit = async (tierOverride?: string) => {
        ...
        setTimeout(() => {
            ...
            setSubmitting(false);
            // Do not redirect, just show the success state in-place (handled by rendering logic)
        }, 1500);
    };
                                ...
                                <div className="grid md:grid-cols-2 gap-6">
                                    ...
                                    <div className="space-y-3 p-5 rounded-2xl bg-slate-800/30 border border-slate-700/30">
                                        ...
                                        <div className="flex gap-2 text-[10px] font-bold text-amber-300 uppercase mt-2">
                                            <span className="px-2 py-1 bg-amber-950/50 rounded">{t('riskAssessment.tagFirstLoss')}</span>
                                            <span className="px-2 py-1 bg-amber-950/50 rounded">{t('riskAssessment.tagHighYield')}</span>
                                        </div>
                                    </div>
                                </div>
