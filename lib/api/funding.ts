...
class FundingAPI {
    ...
    async listPools(page: number = 1, perPage: number = 10): Promise<APIResponse<ListPoolsResponse>> {
        const rawRes = await this.request<FundingPool[] | ListPoolsResponse>(`/marketplace?page=${page}&per_page=${perPage}`);
        ...
        return rawRes as APIResponse<ListPoolsResponse>;
    }

    async getPool(id: string): Promise<APIResponse<FundingPool>> {
        return this.request<FundingPool>(`/pools/${id}`);
    }
